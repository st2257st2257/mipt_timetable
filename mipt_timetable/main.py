...
timetables = [make_timetable(file_name=f"res_data{i}.xlsx", sheet_name=f"{i} курс") for i in range(1, 7)]
print(len(timetables))
audiences = [get_audiences(timetables[j]) for j in range(6)]

last_dicts = []
for my_audience in audiences:
    # print(my_audience)
    print(f"LEN: {my_audience.get_len()}")
    last_dicts.append(my_audience.get_last_dict())

    # my_dict = my_audience.get_last_dict()
    # for aud in my_dict.keys():
    #     print(f"{aud}\t{my_dict[aud]}")

final_last_dict = {}
pair_name_arr[''] = -1
for my_dict in last_dicts:
    for audience_name in my_dict.keys():
        for week_day in week_days:
            if audience_name in final_last_dict.keys() and audience_name != ' ':
                # print(final_last_dict)
                if pair_name_arr[my_dict[audience_name][week_day]] > pair_name_arr[final_last_dict[audience_name][week_day]]:
                    final_last_dict[audience_name][week_day] = my_dict[audience_name][week_day]
            else:
                final_last_dict[audience_name] = {
                    'Понедельник': '',
                    'Вторник': '',
                    'Среда': '',
                    'Четверг': '',
...
for key in final_last_dict.keys():
    _ = key  # str(final_last_dict[key])
    # print(_)
    if (_[-2:] == "ГК") and (len(_) == 6) and (_[0] not in ["0", "1", "2", "3"]):
        print(f"{key + ' '*(12 - len(key))} {final_last_dict[key]}")
        for week_day in dict_for_print.keys():
            if dict_for_print[week_day] == {}:
                dict_for_print[week_day] = {
                    "1705 - 1830": [],
                    "1530 - 1655": [],
                    "1355 - 1520": [],
                }
            if final_last_dict[key][week_day] == '1705 - 1830' and key not in dict_for_print[week_day]['1705 - 1830']:
                dict_for_print[week_day]['1705 - 1830'].append(key)
            elif final_last_dict[key][week_day] == '1530 - 1655' and key not in dict_for_print[week_day]['1530 - 1655']:
                dict_for_print[week_day]['1530 - 1655'].append(key)
            elif pair_name_arr[final_last_dict[key][week_day]] <= pair_name_arr['1355 - 1520'] and key not in dict_for_print[week_day]['1355 - 1520']:
                # elif final_last_dict[key][week_day] == '1355 - 1520' and key not in dict_for_print[week_day]['1355 - 1520']:
                dict_for_print[week_day]['1355 - 1520'].append(key)

for week_day in dict_for_print.keys():
    print(f"\t{week_day}")
    for time_slot in dict_for_print[week_day].keys():
        print(f"\t{week_day}\t{time_slot} and LEN:{len(dict_for_print[week_day][time_slot])}")
        print(dict_for_print[week_day][time_slot])
        string = ''
        for number in dict_for_print[week_day][time_slot]:
            string += f"{number[:3]} "
        print(string)
